% CMStrajectoryVEEGA.m
% Plots the trajectory of the Europa Clipper Spacecraft
% Under two different trajectory assumptions
...
%tau1 = 35178000; %days*86400 sec SR
%exitAng = 100.5*pi/180;
%exitVec = 13900.*[cos(exitAng) sin(exitAng) 0];
...
[rx0, ry0, rz0, tend] = RKballistic3D(tau1,v0,r0);
...
exitAng = 17.56*pi/180;
exitVec = -11000.*[cos(exitAng) sin(exitAng) 0];
JEXIT = exitVec;%q2
JV=JEncVel;%q2
...
%{
%% Titan Encounter 
TEnc = 1.495978707e11.*[-9.484810691136271E+00 -3.554614227471730E-01 3.816915777336816E-01];
TEncVel = [-1.088105567508353E-03 -2.776451725957993E-03 -1.253243749905426E-03];



tau4 =893*86400;
exitAng = 73.3*pi/180;
exitVec = 12000.*[cos(exitAng) sin(exitAng) 0];

r0 = SEnc;
r0(3) = 0;  % flatten the solar system for conveniance
v0 = 1.73145684e6.*[TEncVel(1) TEncVel(2) 0] + exitVec; %AU per day in m/s multiply by 1.73145684e6
[rx3, ry3, rz3, tend] = RKballistic3D(tau4,v0,r0);

figure(1)
plot(rx3,ry3,'b', 'LineWidth',2)
hold on
%}
...
%{
for i = 1:length(rx3)
    OrbDist(i+offset,1) = sqrt(rx3(i)^2+ry3(i)^2); %distance at each point in the orbit
    OrbDist(i+offset,2) = (i-1)*1000+tau1+tau2+tau3;
    Exp(i+offset) = 1000./((OrbDist(i+offset,1)/1.495978707e11)^2);
end
%}
TotalExpVEEGA = sum(Exp)./86400;   %Total 1-AU days of exposure in the DIRECT Trajectory
figure(1)
...
set(gca,'color',[0 0 0])
axis tight
title('Heliocentric Distance versus Time, VEEGA','FontSize',24)
...
%% Question 2
Vgain= norm((JEXIT+JV)-(JENTRY+JV)); % I know that the JV's cancel 
disp(sprintf('\nHeliocentric velocity change at Jupiter is %s m/s\n', num2str(Vgain)));
%% Question 3 
JupiterVesc = 59500 %from https://nssdc.gsfc.nasa.gov/planetary/factsheet/jupiterfact.html
V2=norm(JEXIT)
V1=norm(JENTRY)

deltaV1 = -JupiterVesc+sqrt(JupiterVesc^2+V2^2+V1^2-2*V1*V2);
deltaV2 = -JupiterVesc-sqrt(JupiterVesc^2+V2^2+V1^2-2*V1*V2);
% deltaV1 = -JupiterVesc+sqrt(JupiterVesc^2+V2^2);
% deltaV2 = -JupiterVesc-sqrt(JupiterVesc^2+V2^2);
if deltaV1 > 0
    fprintf('\nΔV at Jupiter is %s m/s\n', num2str(deltaV1));
end
if deltaV2 > 0
    fprintf('\nΔV at Jupiter is %s m/s\n', num2str(deltaV2));
end
